...
def main():
    ...
    for entry in daily_data:
        ...
        if month not in monthly_stats:
            ...
            # So the first occurrence in the loop (latest date) is the monthly close.
            # But let's check date just in case.
    ...
    with open('data/qqq-history.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    print(f"Successfully processed {len(results)} months of data.")
    print("Result saved to data/qqq-history.json")
